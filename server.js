...
async function callAI(messages, provider, model) {
  ...
  if (provider === "groq") {
    ...
    body    = { model: "nvidia/llama-3.1-nemotron-70b-instruct", messages, temperature: 0.6, max_tokens: 2048, stream: false };
  } else if (provider === "deepseek") {
    const key = process.env.DEEPSEEK_KEY;
    if (!key) return "⚠ DEEPSEEK_KEY not set.";
    url     = "https://api.deepseek.com/chat/completions";
    headers = { Authorization: `Bearer ${key}`, "Content-Type": "application/json" };
    body    = { model: "deepseek-chat", messages, temperature: 0.7, max_tokens: 4096, stream: false };
  } else {
    // OpenRouter (default)
    ...
    const chosenModel = model && FREE_MODELS.includes(model) ? model : "meta-llama/llama-3.3-70b-instruct:free";
    url     = "https://openrouter.ai/api/v1/chat/completions";
    headers = { Authorization: `Bearer ${key}`, "Content-Type": "application/json", "HTTP-Referer": "https://aria-69jr.onrender.com", "X-Title": "ARIA" };
    body    = { model: chosenModel, messages, max_tokens: 4096 };
  }

  const response = await fetch(url, { method: "POST", headers, body: JSON.stringify(body) });
  if (!response.ok) {
    const hints = { 401: "API key invalid.", 429: "Rate limit hit. Switch providers.", 500: "Server error." };
...
Use <think>...</think> blocks to reason through each step before acting.`;
      ...
      const result = await runAgenticPipeline(messages, provider || "openrouter", null, true);
