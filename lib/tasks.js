...
import fs from "fs";
import { splitThinking } from "./think.js";
...
async function _executeSteps(task) {
  ...
  for (let i = 0; i < task.steps.length; i++) {
    // Task was deleted mid-run — stop silently, don't broadcast ghosts
    if (!_tasks.has(task.id)) return;
    ...
    try {
      let output;
      try {
        output = await _stepRunner(task, step, prevOutputs);
      } catch (e) {
        // Rate-limited providers (429) recover after a short wait — retry once
        if (!/429|rate.?limit/i.test(e.message)) throw e;
        _logTask(task, `⏳ ${step.title}: rate-limited, retrying in 20s`);
        broadcast({ type: "updated", task });
        await new Promise((r) => setTimeout(r, 20_000));
        if (task.cancelled || !_tasks.has(task.id)) throw e;
        output = await _stepRunner(task, step, prevOutputs);
      }
      // A reasoning model puts its scratchpad in the same string as the
      // answer. Stored raw, every step in the panel opened with a few hundred
      // words of deliberation — so the two are separated here, once, for every
      // step runner rather than in each of them.
      const split = splitThinking(output);
      step.output = split.text || String(output ?? "");
      step.thinking = split.thinking || null;
      step.status = "done";
      step.completedAt = Date.now();
      prevOutputs.push({ title: step.title, output: step.output });
      _logTask(task, `✓ ${step.title}`);
    } catch (e) {
      step.status = "error";
    ...
    if (!_tasks.has(task.id)) return; // deleted while this step ran
    broadcast({ type: "updated", task });
    scheduleSave();
  }

  if (!_tasks.has(task.id)) return; // deleted while last step ran
  task.status = "done";
  task.currentStep = -1;
  ...
  if (task.schedule?.cron) {
    ...
    if (task.schedule.nextRun) {
      ...
      // Cron never matches within the 7-day scan window — surface it
      // instead of leaving a "scheduled" task that silently never fires
      _logTask(task, `⚠ Cron "${task.schedule.cron}" has no upcoming match — check the expression`);
    }
    broadcast({ type: "updated", task });
    scheduleSave();
  }
...
export function resumeTask(id) {
  ...
  if (!t.steps || t.steps.length === 0) {
    // Recovered mid-planning from a restart — no steps yet, so re-plan
    _runTask(t).catch((e) => _markError(t, e.message));
  } else {
